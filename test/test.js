...
describe('Emitter', function () {
  ...
  describe('.once(event, listener)', function () {
    it('should call listener only one time', function () {
      ...
      assert(bus._events['something2'].length === 2);

      bus.emit('something2');

      assert(bus._events['something2'] === undefined);
    });
  });
  ...
  describe('.removeAllListeners(event)', function () {
    ...
    it('should remove all listener for an event', function () {
      bus.on('something', listener);
      bus.on('something', listener2);
      bus.removeAllListeners('something');
      bus.emit('something');
  ...
  describe('.listeners()', function () {
    ...
    it('should recibe a undefined event as parameter and return undefined', function () {
      assert(bus.listeners('something') === undefined);
    });

    it('should return a collection', function () {
      bus.on('something', listener);
      assert(typeof bus.listeners('something') === 'object');
